...
const MOVIE_REVIEW_PROGRAM_ID = "5LjEwmfSxii4aHtvr7iQrxtEiAuX3YGBHKHijgBpFCz1";
...
const MovieReviewForm = () => {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    if (!publicKey)
      return toast.error("Please connect your wallet to submit a review");
    if (processing)
      return toast.error("Please wait for the previous review to complete");

    if (title.trim().length < 1)
      return toast.error("Please enter a valid movie title");
    if (review.trim().length < 1)
      return toast.error("Please enter a valid review title");
    if (parseInt(rating) < 0 || parseInt(rating) > 5)
      return toast.error("Please enter a valid rating (0-5)");
