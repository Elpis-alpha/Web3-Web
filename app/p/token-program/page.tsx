...
import {
  findOrCreateAssociatedTokenAccountTransaction,
  getAcountTokensBalance,
  getTokenAccountWithMint,
} from "@/source/controllers/web3.helpers";
...
export default function Home() {
    ...
    const process = async () => {
      ...
      if (!publicKey) {
        ...
        try {
          ...
          const { associatedTokenAddress, accountInfo } =
            await getTokenAccountWithMint({
              connection,
              publicAddress: publicKey,
              mint: mintInfo,
            });
          const tokenBalance = await getAcountTokensBalance({
            connection,
  ...
  const saveTokenAddress = async (addr: PublicKey) => {
    ...
    if (!publicKey)
      throw new Error("Please connect your wallet to create a token address");
    ...
    if (associatedTokenAddress.toBase58() !== addr.toBase58())
      throw new Error("Token account does not match");
    setAssociatedTokenAddress(associatedTokenAddress);
    tbs[1](tokenBalance);
    setStage("mint-tokens");
  };
    ...
    <main className="flex-1 flex flex-col items-center justify-center gap-5 px-6 py-5 text-sm sm:text-base">
      <div className="w-full flex items-center justify-center">
        ...
        {stage === "connect-wallet" && (
          <div>Connect your wallet to run the token program</div>
        )}
        {stage === "create-mint" && <CreateMint saveMint={saveMint} />}
        {stage === "create-token-account" && (
          <CreateTokenAccount mint={mint} saveTokenAddress={saveTokenAddress} />
        )}
        {stage === "mint-tokens" && (
          <MintTokens
            mint={mint}
            goToBurn={() => setStage("burn")}
            goToTransfer={() => setStage("transfer-tokens")}
          />
        )}
        {stage === "transfer-tokens" && (
          <TransferTokens
            mint={mint}
            goToBurn={() => setStage("burn")}
            goToMint={() => setStage("mint-tokens")}
      ...
      <button
        ...
        className="bg-[#9514ff] text-xs text-white px-2 py-1 rounded-md flex items-center shake"
      >
        Program Info
      </button>
      {showProgramInfo && (
        <ProgramInfo
          mint={mint}
          hideProgramInfo={() => setShowProgramInfo(false)}
          saveMint={saveMint}
