...
export default function Home() {
  ...
  const handleTransfer = async (e: any) => {
    ...
    try {
      ...
      if (typeof error?.message === 'string' && error?.message?.length > 0) {
        if (error.message === 'Non-base58 character') toast.error('Invalid recipient address')
  ...
  return (
    <main className="flex-1 flex flex-col items-center justify-center gap-5 px-6 py-5 text-sm sm:text-base">
      ...
      {!successString && <form onSubmit={handleTransfer} className="max-w-[400px] w-full flex flex-col gap-3">
        <div className="w-full">
          <label htmlFor="recipientAddress" className="block font-medium mb-1 text-sm">Recipient Address:</label>
          <input
            required
            type="text"
            id="recipientAddress"
        ...
        <div className="w-full">
          <label htmlFor="amount" className="block font-medium mb-1 text-sm">Amount:</label>
          <input
            required step="any"
            type="number"
            id="amount"
        ...
        <button className="bg-dark-blue hover:bg-dark-blue-hover text-white font-bold py-2 px-10 rounded mr-auto" disabled={!connected || loading}>Send</button>
      </form>}

      {successString && <h3 className="font-bold text-2xl text-[#29a325]">Transfer successful</h3>}
